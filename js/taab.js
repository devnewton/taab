        ...
        methods: {
            post: function (e) {
                if (this.handleCommand()) {
                    ...
                    formData.set('room', this.room);
                    formData.set('lastId', self.getLastId());
                    var headers = {};
                    if(localStorage.info) {
                        headers['User-Agent'] = localStorage.info;
                    }
                    if(localStorage.token) {
                        headers['Authorization'] = 'Byssus ' + localStorage.token;
                    }
                    fetch("post.php", {
                        method: 'POST',
                        body: formData,
                        ...
                        if (response.ok) {
                            self.parseBackendResponse(response);
                            self.message = "";
                        }
                    });
                }
            ...
            get: function () {
                ...
                fetch("get.php" + params).then(function (response) {
                    if (response.ok) {
                        self.parseBackendResponse(response);
                    }
                });
            ...
            parseBackendResponse: function (response) {
                var room = this.room || '';
                var responseRoom = response.headers.get("X-Room") || '';
